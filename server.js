...
import serviceAccountKey from "./full-stack-blog-website-firebase-adminsdk-r30um-c029f6e770.json" assert{type:"json"}
...
//     api_secret: process.env.API_SECRET_CLOUDNARY
...
server.post("/signup", (req,res) => {
    ...
    if(fullname.length < 3){
        return res.status(403).json({"error": "Fullname must be at leat 3 letter"})
    }
    if(!email.length){
    ...
    if(!passwordRegex.test(password)){
        return res.status(403).json({"error": "Password should be 6 to 20 character long with 1 numeric, 1 lowercase, 1 uppercase"})
    }
    bcrypt.hash(password, 10, async(err, hashed_password) =>{
        ...
        .catch(err =>{
            if(err.code == 11000){
                return res.status(500).json({"error": "Emaail already exist"})
            }
            return res.status(500).json({"error": err.message})
        })
    })
...
server.post("/google-auth", async(req, res) =>{
    ...
    .then(async(decodedUser) =>{
        ...
        let user = await User.findOne({"personal_info.email": email}).select("personal_info.fullname personal_info.profile_img personal_info.username google_auth").then((u) =>{
            return u || null
        })
        .catch(err =>{
            return res.status(500).json({"error": err.message})
        ...
        if(user){
           if(!user.google_auth){
            return res.status(403).json({"error": "This email was singed up without google, please log in with password to access the account" })
           }
        }
        else{
            let username = await generateUsername(email)
    ...
    .catch(err =>{
        return res.status(500).json({"error": "Failed to authanticate you with google. try with some other google account "})
    })
})
...
server.post("/create-blog", verifyJWT, (req, res) =>{
     ...
     let { title, content, banner, tags, des, draft, id } = req.body;

     if(!title.length){
        return res.status(403).json({"error": "You must provide a tittle"})
     }
     ...
     if(!draft){   
         ...
         if(!tags.length || tags.length > 10){
            return res.status(403).json({"error": "You must provide at least 1 tags to published it "})
         }
...
server.post("/like-blog", verifyJWT, (req, res) =>{
    ...
    let {_id, islikedByUsers} = req.body;

    let incrementval = !islikedByUsers ? 1 : -1;
...
server.post("/add-comment", verifyJWT, (req, res) =>{
   ...
   new Comment (commentObj).save().then(async commentFile => {
    ...
    new Notification(notificationObj).save().then(notificationObj => console.log("new notification created"));
